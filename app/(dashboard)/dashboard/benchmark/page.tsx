...
const GENRE_OPTIONS = Object.entries(TMDB_GENRES).map(([id, name]) => ({
  value: id,
  label: name,
}));
...
const formSchema = z.object({
  ...
  genreId: z.coerce.number({ invalid_type_error: "Please select a genre" }).int().positive("Please select a genre"),
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <FormControl>
                        ...
                        >
                          {field.value
                            ? GENRE_OPTIONS.find((g) => g.value === field.value?.toString())?.label
                            : "Select a genre"}
                          ...
                          <CommandGroup>
                            {GENRE_OPTIONS.map((genre) => (
                              <CommandItem
                                key={genre.value}
                                value={genre.label}
                                onSelect={() => field.onChange(genre.value)}
                              >
                                <Check
                                  className={cn(
                                    "mr-2 h-4 w-4",
                                    field.value?.toString() === genre.value ? "opacity-100" : "opacity-0"
                                  )}
                                />
                                {genre.label}
                              </CommandItem>
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} value={field.value}>
                    ...
                    <SelectContent>
                      {["Malay", "English", "Mandarin", "Tamil", "Other"].map(
                        (v) => (
                          <SelectItem key={v} value={v}>
                            {v}
                          </SelectItem>
